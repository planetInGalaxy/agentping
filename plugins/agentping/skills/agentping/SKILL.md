...
```bash
node scripts/config.mjs set-summary-range 15 35
...
- Also suppress `source: exec` sessions whose full session history explicitly declares an isolated/intermediate subtask and says it is not final delivery. Check the whole session because a resumed turn may omit the original declaration. Do not suppress ordinary standalone `codex exec` tasks.
...
- Prompt automatic summaries toward the configured `summaryMinChars` to `summaryMaxChars` range; defaults are 15 to 35 Chinese characters.
